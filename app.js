...
const isLoggedIn = (req, res, next) => {
  ...
  req.user ? got(req,res,next) : res.sendStatus(401);
};
function got(req,res,next){
...
app.post("/auth/local/signup", async (req, res) => {
  ...
  try {
    ...
    req.flash("error", "Error creating a new account[DUPLICATE EMAIL ADDRESS]. Try a different login method.");
    res.redirect("/local/signup")
  }
});
app.get("/api/check",isLoggedIn,(req,res)=>{
  res.send("SUCCESS JWT")
})
